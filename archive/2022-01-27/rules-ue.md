...
Many of these sources are cited throughout this document, but may not be comprehensive.
...
A Hero can dismount from a Vehicle before the Vehicle Activates by performing a [Move or March Move Action](#move-march-move-actions) to move away from the Vehicle. If a Vehicle's pilot dismounts, they may not remount and the Vehicle is considered destroyed.
...
*Note:* [Passengers](#passenger-attacks-skills) may not take a Sustained Attack Action. ![Source: DustEdits](images/source-dustedits.png)
...
In other words, LoS only penetrates 1 square into trees, blocking LoS for squares beyond, and may grant "corner cover" in the same manner as terrain which blocks LoS. ![Source: DustEdits](images/source-dustedits.png)
...
*Note:* Vehicles with the [Small Vehicle](#small-vehicles) property do not block line of sight. ![Source: DustEdits](images/source-dustedits.png)
...
#### Special Actions from within a Fortification
...
See also: [Skill->Ammo Supply](#ammo-supply) (a skill affecting limited ammo weapons)
...
*See also: [Individual Unit Card Errata & FAQ](#individual-unit-card-errata-faq)->[Guaï-Lo Moloï](#guai-lo-moloi)*  
...
When a Unit takes Damage from a Tesla Weapon, it receives a Stunned token. A Unit with a Stunned token cannot fight, including fighting back with a Retaliatory Attack if Attacked in [Close Combat](#8-resolve-close-combat).
...
An Aircraft can never Control or Contest [Objectives](#objectives).
...
### Artillery Observer
One of the greatest advantages of [Artillery](#artillery-weapons) is that it does not need to see its Target. It can use an Observer to radio back the coordinates of the enemy then fire at them from afar.

A Unit with this Skill can use friendly Artillery Weapons as long as the miniature modeled with the Radio is in play. ![Source: 2019 Errata and FAQ](images/source-faq.png)
...
### Assault Shield
**Source: Kowalski's Marauders** 
An Assault Shield is an equipment that gives access to the Skill of the same name.

When a hit is assigned to a miniature with an Assault Shield, roll a die. On a ![Bloc Symbol](images\die-allies-bloc.png), cancel the hit.

In the Steel Guards Close Combat Squad, every miniature is equipped with an Assault Shield. In the Steel Guards Anti-Tank Squad and Steel Guards Tesla Squad only the Unit Leader is equipped with one. You can elect to assign him hits during a Close-Combat Attack. If the Assault Shield roll fails, the Leader is lost, and so is his Assault Shield (don't forget any [Damage Resilient](#damage-resilient) rolls though).

If a Hero with an Assault Shield is assigned to a unit, they may be assigned hits in the same fashion as described above. ![Source: Dust Unofficial](images/source-dustunofficial.png) 
...
This skill applies even if they Hero is off the table such as on the first turn in typical games, or placed in [Reserve](#reserve) on a later turn. ![Source: Discord](images/source-discord.png)
...
(from [DustUSA.net](https://web.archive.org/web/20210710115821/https://static1.squarespace.com/static/5c4a2769af2096c6962e0360/t/5e553e4bd21e26280b9268ef/1582644817363/AL-PLANES0.pdf))
...
## C
### Camouflage
A Unit with the Camouflage Skill may take a Camouflage Special Action. Once it has done so, it cannot be attacked at Range 3 or higher. This benefit ends if the Camouflaged Unit takes any Action other than a [Move](#move-march-move-actions) or [Nothing](#nothing-action) Action. ([Artillery Observers](#artillery-observer) can use their Skill to attack with an [Artillery Unit](#artillery-weapons) as well). The Unit starts the game Camouflaged. A Unit with the Camouflage Skill does not share it with any Unit they Join nor with Heroes that Join this Unit.

*Note:* The [March Move](#move-march-move-actions) action is distinct from a Move action and taking the March Move action will cause a Camouflaged unit to lose the benefits of the skill. ![Source: DustEdits](images/source-dustedits.png)

Camouflaged units to be able to be damaged by [Artillery](#artillery-weapons) through indirect fire. ![Source: Discord](images/source-discord.png)

[Improved Camouflage](#improved-camouflage) grants a similar benefit but enemy units must be within Range 1 to have [Line of Sight](#2-line-of-sight) to the Unit with Improved Camouflaged.
...
*Note:* The single die is added to the roll *after* calculating the number of attack dice generated from a blast weapon - one die total, not one die per model per blast icon. ![Source: DustEdits](images/source-dustedits.png)

### Close Wounds
![Source: Operation Hellgate](images/source-hellgate.png)  
A Unit with this skill may take a ***Close Wounds Special Action***. If they do so, the Player rolls a die. On a roll of ![Bloc Symbol](images\die-allies-bloc.png) or ![Crosshair Symbol](images\die-allies-crosshair.png), the Player selects one Unit with the [Lesser Mythos Creature](#lesser-mythos-creature) or [Mythos Creature](#mythos-creature) Skill that is within Range 1 of the Unit using this Skill. If the selected Unit has a Vehicle or Aircraft Armor, remove one Damage from it's Damage Track. If the selected Unit has Soldier Armor, return one eliminated miniature to the Unit.

A Unit may perform this Skill on itself.

See also: Skills->[High Priest](#high-priest), [Psychic Choir](#psychic-choir)
...
### Commissar
A lone Commissar must Join an Infantry Unit with the same Armor value at the start of the game, becoming part of the Unit for the rest of the game. Squads and Heroes are Infantry Units. ![Source: 2019 Errata and FAQ](images/source-faq.png)
...
A Squad from a specific Faction Joined by a Commissar is always considered belonging to the said Faction. ![Source: 2019 Errata and FAQ](images/source-faq.png)

*See also: [Individual Unit Card Errata & FAQ](#individual-unit-card-errata-faq)->[Commissar Poon](#commissar-poon)*
...
### Cultist Command Squad
![Source: Hellgate](images/source-hellgate.png) 
This specialized version of a [Command Squad](#command-actions) is specific to the Mythos Bloc. It contains an [Officer](#officer-special-actions), [Mechanic](#mechanic-special-action), and [Medic](#medic-special-actions) that are identical to those of a regular Command Squad. Their skills can only affect Mythos Units that are not [Mythos Creatures](#mythos-creature) or [Lesser Mythos Creatures](#lesser-mythos-creature).

The squad also contains two Priests that form a [Psychic Choir](#psychic-choir). If at least one Priest remains in the Unit and on the battlefield, a [High Priest](#high-priest) (joined with the squad) can use the Psychic Choir ability.
...
A Unit with this skill always benefits from Damage Resilient when in Cover, even against weapons that negate Cover (including [Close-Combat Attacks](#8-resolve-close-combat)). ![Source: 2019 Errata and FAQ](images/source-faq.png)
...
### Devour
![Source: Dust Enlist App](images/source-dustenlist.png)  
Each time this Hero inflicts one point of Damage in [Close Combat](#8-resolve-close-combat), they heal one Health.

A Unit with this Skill cannot have more Health than the maximum written on its [Unit Card](#reading-unit-cards). Since Close Combat is simultaneous (except against [First Strike](#first-strike)), a Unit with Devour can heal itself while taking Damage. ![Source: 2019 Errata and FAQ](images/source-faq.png)
...
### Fanatical
![Source: Hellgate](images/source-hellgate.png)  
A hero with this Skill can never receive an Under Fire or [Suppression](#7-hit-units-take-suppression) Token. This Skill applies with teh Hero Joins a Squad or is piloting a Vehicle or an Aircraft.
...
Applies to the entire attack action, including all weapons on a piloted vehicle. ![Source: Discord](images/source-discord.png)
...
If a First Strike Attack or Sustained Attack places a Stun Token (thanks to a Skill or Equipment, for example) on the target Unit without First Strike, the Stunned Unit can't retaliate since a Stunned Unit cannot fight in Close Combat. ![Source: 2019 Errata and FAQ](images/source-faq.png)
...
Only a Flying Unit can Join a Flying Unit. ![Source: 2019 Errata and FAQ](images/source-faq.png)
...
### Get Moving You Monster
![Source: Operation Hellgate](images/source-hellgate.png)  
***Get Moving You Monster Special Action*** is almost identical to the normal [Get Moving You Bunch of Monkeys](#get-moving-you-bunch-of-monkeys) except that it may only affect [Mythos Creatures](#mythos-creature) and [Lesser Mythos Creatures](#lesser-mythos-creature), and is limited to **Range 1** without use of the [Psychic Choir](#psychic-choir).

*See also: Skill->[High Priest](#high-priest), [Close Wounds](#close-wounds)*
...
### High Priest
![Source: Dust Enlist App](images/source-dustenlist.png) ![Source: Operation Hellgate](images/source-hellgate.png)  
Can use the [Get Moving You Monster](#get-moving-you-monster) and [Close Wounds](#close-wounds) Special Actions on a [Mythos Creature](#mythos-creature) or [Lesser Mythos Creature](#lesser-mythos-creature).

*Note:* [Get Moving You Monster](#get-moving-you-monster) is almost identical to the normal [Get Moving You Bunch of Monkeys](#get-moving-you-bunch-of-monkeys) except that it only affects Mythos Creatures (including Lesser), and is limited to **Range 1** without the [Psychic Choir](#psychic-choir). ![Source: DustEdits](images/source-dustedits.png)
...
### Interrogate
If a Unit with this Skill (or a Unit Joined) uses a Close Combat Weapon to eliminate a Hero or Officer, or [Execute](#execute) (skill) a Hero or Officer, roll four dice for [initiative](#initiative) at the start of each Turn for the rest of the game.

## J
### Jammer
All successful dice rolls for Actions performed by an enemy Unit using a Radio (transmitting ![Source: Discord](images/source-discord.png)) within Range 6 of a Unit equipped with a Jammer are re-rolled. Enemies in [Command Vehicles](#command-vehicle) must re-roll successful dice rolls for each attempt to perform an [Action using a Radio](#command-actions).
...
![Source: Dust Enlist App](images/source-dustenlist.png)  
Can never be the target of any [Command Squad](#command-actions), [Officer](#officer), or [Medic](#combat-medic) Actions.

*See also: Skills->[Mythos Creature](#mythos-creature), [High Priest](#high-priest), [Psychic Choir](#psychic-choir)*
...
### Mythos Creature
![Source: Dust Enlist App](images/source-dustenlist.png)  
Immune to [Suppression](#7-hit-units-take-suppression) and [Critical Hits](#critical-hits).

See also: Skills->[Lesser Mythos Creature](#lesser-mythos-creature), [High Priest](#high-priest), [Psychic Choir](#psychic-choir)
...
### Phosphorus
See: [Special Weapons->Phosphorus Weapons](#phosphorus-weapons)

### Pilot
May mount a (ground) [Vehicle](#types-of-units) and use Skills while mounted.
...
### Psychic Choir
![Source: Dust Enlist App](images/source-dustenlist.png) ![[Source: Hellgate](images/source-hellgate.png)] 
Allows a [High Priest](#high-priest) to use its two Special Actions to be used on [Mythos Creatures](#mythos-creature) at a distance.

The Cultist Command Squad contains two Priests that form a Psychic Choir. If at least one Priest remains in the Unit and on the battlefield, a High Priest can use the Psychic Choir ability.

![Source: Discord](images/source-discord.png)  
This skill acts much like an officer using a [radio](#officer-special-actions), except that the either of the (regular) Priests provide the Psychic Choir (Radio-esque) skill - most other Armor 1 & 2 command squads only have one radio operator. Note: the High Priest themselves does not have the Psychic Choir ability.

See Also: [Get Moving You Monster](#get-moving-you-monster), [Close Wounds](#close-wounds)

### Psychic Scream
![Source: Dust Enlist App](images/source-dustenlist.png)  
Once per game, execute Psychic Scream Free Action to Attack all Units, friends or foes, in Range 3 radius. Ignores Line of Sight, [Infantry Saves or Cover Saves](#5-target-units-roll-saves).

### Psychic Vortex
![Source: Dust Enlist App](images/source-dustenlist.png)  
Once per game, execute Psychic Vortex Free Action to Attack all Units, friends or foes, in Range 2 radius. Ignores Line of Sight, [Infantry Saves or Cover Saves](#5-target-units-roll-saves).
...
### Scout
If a Unit with this Skill (and any Unit it has Joined or Hero that has Joined it) performs a March Move during its first Activation of the game, it gains a third Action for this Activation. ![Source: 2019 Errata and FAQ](images/source-faq.png)
...
### Small Ammo Dump
![Source: Dust Enlist App](images/source-dustenlist.png)  
One Ground Unit adjacent to this Unit can [Reload](#reload-weapons) as a Free Action.

This benefit may be used by only one Unit per full game turn, and is not limited to friendly units - first come first serve! ![Source: Discord](images/source-discord.png)

*Note: Confusingly, this skill has the same name as the [Special Terrain->Small Ammo Dump](#small-ammo-dump) but with a completely different benefit. The terrain affects [Limited Ammo Weapons](#limited-ammo-weapons), whereas the skill affects [Reload](#reload-weapons) and [Salvo](#salvo-weapons) Special Weapons.* ![Source: DustEdits](images/source-dustedits.png)

See also: [Skills->Ammo Dump](#ammo-dump) (no *once-per-turn* limitation of the skill)
...
*Note:* Most vehicles which occupy a single square are **Standard** size. To benefit from this skill, **Small** must be listed under the skills section of the Unit Card. ![Source: DustEdits](images/source-dustedits.png)
...
Vehicles receive a [Cover Save](#cover) if applicable. ![Source: Discord](images/source-discord.png)
...
A Unit with the Spy Skill can choose to:  ![Source: 2019 Errata and FAQ](images/source-faq.png)
...
*Note:* Must be on the battlefield (deployed), unlike the [Black Ops](#black-ops) skill. ![Source: Discord](images/source-discord.png)
...
### Commissar Poon
This SSU character is both a [Hero](#heroes) and a [Commissar](#commissar). As such, he must Join a Unit, but no other Hero or Commissar can be Joined to the same Unit. Unlike other Heroes he can be returned to the game if he is destroyed (as non-Hero Commissars sometimes can through special rules like [Reserves](#reserve) or Platoon Advantages). ![Source: 2019 Errata and FAQ](images/source-faq.png)

### Nina, KV-47D Aero, Light Anti-Tank Walker
The Dual Triple RPG Weapon benefits from the [Grenade](#grenade-weapons) Rule. ![Source: 2019 Errata and FAQ](images/source-faq.png)

### The Dragon & The Phoenix
The printed Unit Cards for these two SSU Heroes mistakenly omits the [Hero](#heroes) designation. These characters are both Heroes. ![Source: 2019 Errata and FAQ](images/source-faq.png)

### Guaï-Lo Moloï
Guaï-Lo's RPG is a [Limited Ammo Weapon](#limited-ammo-weapons): he only has four shots for the entire game. Since he does not possess the [Volley Skill](#volley-weapons), he can only fire one RPG at a time per Attack or Sustained Attack Action. ![Source: 2019 Errata and FAQ](images/source-faq.png)

## SUGGESTED HOUSE RULES ![Source: Dust Unofficial](images/source-dustunofficial.png)   
...
#### *Reasoning:*
Rules as Written state Infantry Units may be placed within Range 1. GregB clarified on ![Source: Discord](images/source-discord.png) that this could be on the far side of impassible objects. Furthermore, Infantry Units could not be placed in the same square of the wreck, despite being able to occupy that square ![Source: 2019 Errata and FAQ](images/source-faq.png).
...
#### *Reasoning:*
This change was made in the ![Source: 2019 Errata and FAQ](images/source-faq.png) and severely restricts how Units with the Flying skill and Heroes can be combined. As a trade-off for allowing these units to combine, they must give up their Flying skill when combined.
...
#### *Reasoning:*
Rules as Written imply rotation is only to used to bring an enemy into a firing arc. If the Enemy Unit is already in a firing arc, but not a preferred one (e.g. an Anti-Infantry weapon rather than Anti-Tank), the unit may rotate (per GregB of DustUSA on the [Dust Discord Server](https://discord.com/invite/MHgSt6R)).
...
Reactive Attacks require forfeiting a normal activation for merely a chance of a limited Reactive Attack, and a Move/Attack reactive action forfeits a Sustained Attack Action. The slight additional leeway in rotation seems to be a fair trade off for the limitations of a Reactive Attack - especially considering this exact situation would be a rare occurrence.
...
#### *Reasoning:*
[Airborne](#airborne) troops would normally deploy anywhere on the battlefield via parachute, which does not fit with the theme during indoor scenarios. The Scout skill preserves some of the rapid deploy feel of these troops, especially in cramped tunnels or interior spaces.

### Vehicles Destroy Ammo Crates
#### *Suggested House Rule:*
[Vehicles](#types-of-units_1) entering a square containing an [Ammo Crate](#ammo-crates) destroy the crate, removing it from play. Vehicles may not receive cover from an ammo crate (because they cannot occupy the same square as the crate without destroying it).

#### *Alternate House Rule:*
Same as above, but only applies to [Walkers](#types-of-units_1) and [Tracked](#types-of-units_1) Vehicles. Any walker or tracked vehicle with the *[Small](#small-vehicles)* property as well as half-tracked vehicles may choose whether to occupy the square with the crate or destroy it. Half-tracked, wheeled, and small vehicles receive cover when occupying the same square as an Ammo Crate.

#### *Reasoning:*
Forcing the destruction of this terrain can make players think more carefully about passing their heavy units through valuable cover-granting squares. Additionally, these rules are how some players already interpreted the rules as written.
